...
export const insertarDetalleCalificacion = async (
  ...
  const calificacion = await AppDataSource.manager.findOne(Calificacion, { 
    where: { id: calificacionId } 
  });
  
  const criterio = await AppDataSource.manager.findOne(CriterioEvaluacion, { 
    where: { id: criterioId } 
  });
